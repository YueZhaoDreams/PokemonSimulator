...
3. **Ask Cursor** — tap the Combo Cub mark (bottom-right). On a phone or tablet the chat covers the whole screen. Split with the stadium is only for a wide desktop mouse layout. Talk/mic and spoken replies stay in that panel. The agent runs in-process lab tools (simulate, odds, decks). It cannot edit the git checkout or write `data/lab/`.
