...
3. **Ask Cursor** — tap the Combo Cub mark (bottom-right). Chat opens as a split panel on the right on desktop, and fullscreen on a phone. Talk/mic and spoken replies stay in that panel. The agent runs in-process lab tools (simulate, odds, decks). It cannot edit the git checkout or write `data/lab/`.
