...
from app.catalog import PREFERRED_IDS, fill_missing_card_image, lookup_seed_card, resolve_name, search_local, fetch_full, normalize_card
...
def api_resolve(payload: dict, _user: dict = Depends(require_user)) -> dict:
    ...
    if cid:
        ...
        if seed:
            return fill_missing_card_image(seed.to_dict())
        try:
            return fill_missing_card_image(normalize_card(fetch_full(cid)).to_dict())
        except Exception:
            pass
    if name:
    ...
    if pin and pin != cid:
        ...
        if seed:
            return fill_missing_card_image(seed.to_dict())
        try:
            return fill_missing_card_image(normalize_card(fetch_full(pin)).to_dict())
        except Exception:
            pass
    try:
        return fill_missing_card_image(resolve_name(name, payload.get("prefer")).to_dict())
    except Exception as exc:
