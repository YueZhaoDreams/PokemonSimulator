...
class Game:
    ...
    def _apply_mulligan_bonus_draws(self) -> None:
        """After prizes and setup, draw one card per opponent mulligan when the flag is on."""
    ...
    def _is_protected_from_energy(self, me: Player, card, strat: StrategySpec) -> bool:
        ...
        if strat.name == "slash":
            if name == "floragato":
                return self._copies_hand_and_play(me, "Floragato") <= 1
            if name == "sprigatito":
    ...
    def _pick_starter(self, player: Player, basics: list[int], strat: StrategySpec) -> int:
        ...
        def score(i: int) -> float:
            ...
            if strat.name == "g" and name == "flutter mane":
                return 1400
            if strat.name == "g" and name == "indeedee":
                return 1850
            if strat.name == "g" and name == "emolga":
                return 1650
            if strat.name == "g" and name == "hop's cramorant":
                return 1550
            if strat.name == "g" and name == "relicanth":
                return 1250
            if strat.name == "g" and name == "iron boulder":
                return 1600
            if strat.name == "g" and name == "plusle":
                # 70 HP closer. Do not open; Emolga / Trapinch / Clefairy should.
                return -200
            if strat.name == "phantom" and name == "budew":
                return 900
            if strat.name == "phantom" and name == "dreepy":
    ...
    def _play_basics(self, me: Player) -> None:
        ...
        def prio(card_i: int) -> tuple:
            ...
            if strat.name == "g":
                ...
                foe = self.players["b" if who == "a" else "a"]
                if self._facing_demolish(me) or self._foe_has_stance(me):
                    rank["plusle"] = 1
                    rank["iron boulder"] = 2
            ...
            if name in {"ultra ball", "poké ball", "poke ball"} and missing_protect:
                ...
                if copies >= max(1, strat.max_ace_copies) and strat.name == "demolish":
                    ...
                    if self._facing_aura(me):
                        # Nest finds a Basic. Vs Lucario that Basic is Clefairy, not Mewtwo.
                        if clefairy_count < self._clefairy_play_cap(me):
                            score += 12
                        else:
                            score -= 6
                    elif self._invitation_held_for_dump(me, who) and have_mewtwo:
                        ...
                        if clefairy_count < self._clefairy_play_cap(me):
                            ...
                            score += 2
                    elif not have_mewtwo:
                        score += 13
                    elif clefairy_count < self._clefairy_play_cap(me):
                        score += 6
                    else:
                        score += 2
                ...
                if strat.name == "party":
                    ...
                    if me.active and not self._hand_has_attachable_energy(me, who):
                        score += 8
                    else:
                        score += 5
                ...
                if strat.name == "slash":
                    ...
                    score += 12 if not have_flora else 2
                elif strat.name == "party" and self._mega_mon(me) is None:
                    score += 8
                else:
                    score += 3
                ...
                score += 6 if strat.name == "g" else 1
            elif name == "potion":
                ...
                if strat.name == "g":
                    boulder_out = any(me.card(m.card_i).name.lower() == "iron boulder" for m in me.in_play())
                    boulder_active = bool(me.active) and me.card(me.active.card_i).name.lower() == "iron boulder"
                    if boulder_active:
                        score -= 20
                    elif boulder_out and self._g_wants_horn_swing(me, foe, who):
                        score += 22 if len(foe.hand) in {4, 5} else 14
                    else:
                        score += 18 if self._g_incoming_idx(me, who) is not None else -8
                else:
    ...
    def _energy_switch_helps(self, me: Player) -> bool:
        ...
        if self.strats[who].name == "g" and self._is_clefairy(card):
            if self._can_pay_wonder_storm(me, me.active):
                return False
            return any(bool(mon.energy) for mon in me.bench)
    ...
    def _resolve_trainer(self, me: Player, foe: Player, card: Card, who: str = "a", card_i: int | None = None) -> None:
        ...
        if name in {"hop"}:
            ...
            if self.strats[who].name == "party" and self._facing_aura(me):
                prefer = ["Clefable ex", "Mega Clefable ex", "Clefable"]
            elif self.strats[who].name == "party" and self._facing_phantom(me):
            ...
            if self.rng.random() < 0.5:
                ...
                if targets:
                    dest = min(targets, key=lambda m: self._max_hp(foe, m) - m.damage)
                    dest.damage += 20
                    self._bump("venture_bomb_heads")
            elif me.active:
                me.active.damage += 20
                self._bump("venture_bomb_tails")
        elif "redeemable ticket" in name:
            n = len(me.prizes)
            me.deck.extend(me.prizes)
            me.prizes = []
            self.rng.shuffle(me.deck)
            me.prizes = [me.deck.pop(0) for _ in range(min(n, len(me.deck)))]
            self._bump("redeemable_ticket")
        elif name == "energy switch":
        ...
        if strat.name == "party":
            ...
            if self._facing_aura(me):
                if not self._has_lunar_zone(me) and not any(
                    self._is_clefable_ex(me.card(i)) for i in me.hand
                ):
                    prefer.append("Clefable ex")
                if self._mega_mon(me) is None and not any(
    ...
    def _energy_search_prefer(self, me: Player, who: str) -> list[str]:
        ...
        if strat.name == "party":
            if self._facing_aura(me):
                if (
                    self._count_named_in_play(me, "Clefairy")
                    + sum(1 for i in me.hand if self._is_clefairy(me.card(i)))
            ...
            return list(dict.fromkeys(prefer))
        if strat.name == "g":
            munk = next((m for m in me.in_play() if me.card(m.card_i).name.lower() == "munkidori"), None)
            if munk is not None and "Darkness" not in self._energy_pool(me, munk):
                prefer.append("Darkness Energy")
            prefer.append("Psychic Energy")
            return list(dict.fromkeys(prefer))
    ...
    def _attach_energy(self, me: Player, who: str) -> None:
        ...
        strat = self.strats[who]
        foe = self.players["b" if who == "a" else "a"]
        if strat.name == "g" and self._g_horn_attach(me, foe, who):
            return
    ...
    def _energy_target(self, me: Player, strat: StrategySpec) -> Pokemon:
        ...
        if strat.name == "party":
            ...
            if self._facing_aura(me):
                return self._party_vs_aura_energy_target(me)
            ...
            if self._want_three_two_combo(me, foe):
                ...
                if main is not None and not self._mewtwo_can_pay_photon(me, main):
                    return main
                support = self._support_mewtwo(me)
                if support is not None and not self._can_pay_transfer(me, support):
                    return support
                if main is not None:
                    return main
            ...
            if self._support_transfer_turn(me, foe):
                if not self._can_pay_transfer(me, me.active):
                    return me.active
                return self._main_mewtwo(me) or me.active
            if (
                me.active
        ...
        if strat.name == "g":
            ...
            plusle = self._plusle_mon(me)
            if plusle is not None:
                paid = can_pay_energy(self._energy_pool(me, plusle), ["Colorless", "Colorless"])
                if (
                    not paid
                    and self._g_plusle_kos(me, foe, plusle, extra_energy=2)
            ...
            if boulder is not None and horn is not None:
                if not can_pay_energy(self._energy_pool(me, boulder), horn.cost) and not self._g_storm_would_ko(me, foe):
                    return boulder
            for mon in me.in_play():
            ...
            for mon in me.in_play():
                if not self._is_clefairy(me.card(mon.card_i)):
                    continue
                if not self._can_pay_wonder_storm(me, mon):
                    return mon
            closers = {n.lower() for n in strat.closers}
            if self._active_can_chip(me, strat):
                for mon in me.bench:
    ...
    def _attack(self, me: Player, foe: Player, who: str) -> None:
        ...
        for effect in atk.effects:
            ...
            if effect.get("kind") == "require_equal_hands" and len(me.hand) != len(foe.hand):
                self._log(f"{attacker.name} used {atk.name} but it did nothing")
                return
            if effect.get("kind") == "require_opponent_prizes":
                allowed = {int(v) for v in (effect.get("values") or [])}
        ...
        for effect in atk.effects:
            ...
            elif effect.get("kind") == "recycle_energy_from_discard":
                self._recycle_energy_from_discard(me, int(effect.get("count") or 2))
            elif effect.get("kind") == "evolve_from_deck":
                self._evolve_from_deck(me, foe, who)
            elif effect.get("kind") == "self_bench_damage":
        ...
        for atk in legal:
            ...
            if any(e.get("kind") == "transfer_charge" for e in atk.effects):
                ...
                if strat.name == "party" and self._want_storm_line(me, foe, who):
                    ...
                    score += 500
                elif strat.name == "party" and not any(
            ...
            if any(e.get("kind") == "call_family" for e in atk.effects):
                ...
                elif strat.name in {"g", "party"} and slots > 0:
                    have = self._count_named_in_play(me, "Clefairy")
                    cap = 3 if strat.name == "g" else self._clefairy_play_cap(me)
                    deck_has = any(self._is_clefairy(me.card(i)) for i in me.deck)
                    if have < cap and deck_has:
                        score += 90
                    elif slots > 0:
                        score += 20
                    else:
            ...
            if any(e.get("kind") == "disable_attack" for e in atk.effects) and foe.active:
                ...
                if best_foe >= 80 and effective < foe_hp:
                    score += 35
            if any(e.get("kind") == "evolve_from_deck" for e in atk.effects):
                score += 110 if self._g_nurturer_target(me) is not None else -25
            if any(e.get("kind") == "recycle_energy_from_discard" for e in atk.effects):
                nrg = sum(1 for i in me.discard if me.card(i).is_energy)
                score += 70 if nrg >= 2 else (25 if nrg else -30)
            if any(e.get("kind") == "require_equal_hands" for e in atk.effects):
                score += 90 if len(me.hand) == len(foe.hand) else -200
            if any(e.get("kind") == "require_opponent_prizes" for e in atk.effects):
                allowed = set()
                for e in atk.effects:
                    if e.get("kind") == "require_opponent_prizes":
                        allowed.update(int(v) for v in (e.get("values") or []))
                score += 80 if len(foe.prizes) in allowed else -90
            if any(e.get("kind") == "damage_one_pokemon" for e in atk.effects):
                amt = next(int(e.get("amount") or 0) for e in atk.effects if e.get("kind") == "damage_one_pokemon")
                score += min(60, amt)
        ...
        def prio(i: int) -> int:
            ...
            if strat.name == "party":
                ...
                if "mega clefable" in name:
                    return 0
                if "mewtwo" in name:
                    # Promote Transfer support (no Belt, less energy) ahead of the main attacker.
                    return 1 + (5 if self._belt_on(player, mon) else 0) + min(len(mon.energy), 3)
                if name == "clefable ex":
                    return 8
                if name == "clefable":
    ...
    def _raw_attack_damage(self, me: Player, foe: Player, mon: Pokemon, atk, extra_discard_n: int | None = None, ignore_hand_gate: bool = False) -> int:
        if not foe.active:
            return 0
        ...
        for effect in atk.effects:
            if effect.get("kind") == "opponent_prize_bonus":
                ...
                op = str(effect.get("op") or "eq")
                have = len(foe.prizes)
                if op == "at_most" and have <= need:
                    dmg += int(effect.get("bonus") or 0)
                elif op != "at_most" and have == need:
                    dmg += int(effect.get("bonus") or 0)
            if effect.get("kind") == "coin_damage_bonus" and self.rng.random() >= 0.5:
                dmg += int(effect.get("bonus") or 0)
            if effect.get("kind") == "energy_in_play_bonus":
                need = int(effect.get("count") or 4)
                ...
                if have >= need:
                    dmg += int(effect.get("bonus") or 0)
            if effect.get("kind") == "attached_named_energy_bonus":
                needle = str(effect.get("name") or "").lower()
                if any(needle in me.card(i).name.lower() for i in mon.energy):
                    dmg += int(effect.get("bonus") or 0)
                ...
                if self._want_mega_rush(me, foe, who):
                    for mon in me.in_play():
                        if mon.tool is None and "mega clefable" in me.card(mon.card_i).name.lower():
                            return mon
                open_mewtwo = [m for m in self._mewtwo_mons(me) if m.tool is None]
                if open_mewtwo:
                    bench = [m for m in open_mewtwo if m is not me.active]
                    return max(bench or open_mewtwo, key=lambda m: len(m.energy))
            for mon in me.in_play():
        ...
        for abi in card.abilities:
            ...
            for eff in self._ability_effects(abi):
                if eff.get("kind") == "gust_low_hp_on_evolve":
                    ...
                    cap = eff.get("max_remaining_hp")
                    if cap is not None:
                        # Same printed sentence as gust_low_hp_on_evolve. Do not gust twice.
                        if not gusted_limited:
                            self._gust_low_hp_bench(foe, int(cap))
                    elif foe.bench:
                        self._boss_orders(me, foe)
    ...
    def _facing_aura(self, me: Player) -> bool:
        """Mega Lucario ex is Psychic-weak. Photon is Lightning; Mewtwo is Fighting-weak."""
        if self._foe_strat_name(me) == "aura":
            return True
        foe = self.players["b" if me.name == "A" else "a"]
        return any(card.name.lower() == "mega lucario ex" for card in foe.cards)
    ...
    def _party_vs_aura_energy_target(self, me: Player) -> Pokemon:
        """Wondrous Moon needs three Psychic. Shooting Moons needs two. Never Mewtwo first."""
        assert me.active
        unpaid_ex = [
            mon
            for mon in me.in_play()
            if self._is_clefable_ex(me.card(mon.card_i)) and not self._can_pay_wondrous_moon(me, mon)
        ]
        if unpaid_ex:
            return max(unpaid_ex, key=lambda mon: len(mon.energy))
        mega = self._mega_mon(me)
        if mega is not None and not self._can_pay_shooting_moons(me, mega):
            return mega
        clefs = [mon for mon in me.in_play() if self._is_clefairy(me.card(mon.card_i))]
        if clefs:
            return max(clefs, key=lambda mon: (len(mon.energy), 0 if mon is me.active else 1))
        exes = [mon for mon in me.in_play() if self._is_clefable_ex(me.card(mon.card_i))]
        if exes:
            return max(exes, key=lambda mon: len(mon.energy))
        if mega is not None:
            return mega
        return me.active

    def _aura_swing(self, foe: Player) -> int:
        if not foe.active:
            return 0
        card = foe.card(foe.active.card_i)
        pool = self._energy_pool(foe, foe.active)
        locked = foe.active.disabled_attack or getattr(foe.active, "disabled_self", None)
        best = 0
        for atk in card.attacks:
            if atk.name == locked:
                continue
            if not can_pay_energy(pool, atk.cost):
                continue
            dmg = int(atk.damage or 0)
            if "Fighting" in (card.types or []):
                dmg += int(foe.fighting_boost or 0)
            best = max(best, dmg)
        return best

    def _aura_incoming(self, foe: Player, card: Card) -> int:
        if not foe.active:
            return 0
        return self._aura_swing(foe) * weakness_multiplier(
            card.weaknesses or [], foe.card(foe.active.card_i).types or []
        )

    def _retreat_party_vs_aura(self, me: Player, foe: Player, who: str) -> None:
        """Psychic closer vs Mega Lucario. Mewtwo only stays when Photon already KOs."""
        if not me.active or not me.bench:
            return
        if self._moon_ko(me, foe):
            return
        for idx, mon in enumerate(me.bench):
            if self._moon_ko(me, foe, mon):
                self._swap_to_bench(me, who, idx, allow_paid=True)
                return
        if self._shooting_moons_ko(me, foe, me.active):
            return
        for idx, mon in enumerate(me.bench):
            if self._shooting_moons_ko(me, foe, mon):
                self._swap_to_bench(me, who, idx, allow_paid=True)
                return

        def hp_left(mon: Pokemon) -> int:
            return self._max_hp(me, mon) - mon.damage

        def incoming(mon: Pokemon) -> int:
            return self._aura_incoming(foe, me.card(mon.card_i))

        def moon_swing(mon: Pokemon) -> bool:
            return (
                self._is_clefable_ex(me.card(mon.card_i))
                and self._can_pay_wondrous_moon(me, mon)
                and hp_left(mon) > incoming(mon)
            )

        if moon_swing(me.active):
            return
        for idx, mon in enumerate(me.bench):
            if moon_swing(mon):
                self._swap_to_bench(me, who, idx, allow_paid=True)
        ...
        def mega_swing(mon: Pokemon) -> bool:
            return (
                "mega clefable" in me.card(mon.card_i).name.lower()
                and self._can_pay_shooting_moons(me, mon)
                and hp_left(mon) > incoming(mon)
            )

        if mega_swing(me.active):
            return
        for idx, mon in enumerate(me.bench):
            if mega_swing(mon):
                self._swap_to_bench(me, who, idx, allow_paid=True)
        ...
        if self._is_mewtwo(me.card(me.active.card_i)):
            ...
            for idx, mon in enumerate(me.bench):
                ...
                if "mega clefable" in name or name == "clefable ex":
                    if self._swap_to_bench(me, who, idx, allow_paid=True):
                        return
            return
        if hp_left(me.active) <= incoming(me.active):
            scored: list[tuple[int, int, int, int, int]] = []
            for idx, mon in enumerate(me.bench):
                survives = 1 if hp_left(mon) > incoming(mon) else 0
                mega = 1 if "mega clefable" in me.card(mon.card_i).name.lower() else 0
                prizes = self._prizes_for_ko(me.card(mon.card_i))
                scored.append((survives, mega, -prizes, hp_left(mon), idx))
            scored.sort(reverse=True)
            if scored and (
                scored[0][0] == 1
                or scored[0][1] == 1
                or -scored[0][2] < self._prizes_for_ko(me.card(me.active.card_i))
            ):
                self._swap_to_bench(me, who, scored[0][4], allow_paid=True)

    def _evolve_party_vs_aura(self, me: Player, who: str) -> None:
        """Clefable ex first (Wondrous Moon), then Mega, keeping a Party Clefairy when Moon is not ready."""
        ...
        def evolve_fueled(evo_name: str) -> bool:
            ...
            if not candidates:
                return False
            candidates.sort(key=lambda mon: (len(mon.energy), 0 if mon is me.active else 1), reverse=True)
            self._do_evolve(me, candidates[0], evo_i)
            return True

        if self._moon_cannon(me) is None:
            evolve_fueled("Clefable ex")
        if self._mega_mon(me) is not None:
            return
        clefs = [mon for mon in me.in_play() if self._is_clefairy(me.card(mon.card_i))]
        cannon = self._moon_cannon(me)
        if cannon is None and evo_in_hand("Clefable ex") is not None:
            return
        cannon_ready = cannon is not None and self._can_pay_wondrous_moon(me, cannon)
        if len(clefs) >= 2 or cannon_ready or cannon is None:
            evolve_fueled("Mega Clefable ex")
    ...
    def _mewtwo_play_cap(self, me: Player) -> int:
        ...
        """
        if self._facing_aura(me):
            # Mega Brave is 270 Fighting. Weakness makes that 540 into Mewtwo.
            return 0
    ...
    def _is_clefable(self, card) -> bool:
        return card.name.lower() == "clefable"
    ...
    def _has_belt_in_hand_or_play(self, me: Player) -> bool:
        if any(self._belt_on(me, m) for m in me.in_play()):
            return True
        return any("maximum belt" in me.card(i).name.lower() for i in me.hand)

    def _zone_piece_available(self, me: Player) -> bool:
        if self._has_lunar_zone(me):
            return True
        return any(self._is_clefable_ex(me.card(i)) for i in me.hand)

    def _fodder_piece_available(self, me: Player) -> bool:
        """Clefable, or a second Clefable ex once Zone is spoken for."""
        if any(self._is_clefable(me.card(m.card_i)) for m in me.in_play()):
            return True
        if any(self._is_clefable(me.card(i)) for i in me.hand):
            return True
        ex_play = sum(1 for m in me.in_play() if self._is_clefable_ex(me.card(m.card_i)))
        ex_hand = sum(1 for i in me.hand if self._is_clefable_ex(me.card(i)))
        need = 1 if self._has_lunar_zone(me) else 2
    ...
    def _is_fodder_mon(self, me: Player, mon: Pokemon) -> bool:
        if self._is_clefable(me.card(mon.card_i)):
            return True
        if not self._is_clefable_ex(me.card(mon.card_i)):
            return False
        return sum(1 for m in me.in_play() if self._is_clefable_ex(me.card(m.card_i))) >= 2

    def _best_fodder_idx(self, me: Player) -> int | None:
        scored: list[tuple[int, int, int, int]] = []
        for idx, mon in enumerate(me.bench):
            if not self._is_fodder_mon(me, mon):
                continue
            is_clefable = 1 if self._is_clefable(me.card(mon.card_i)) else 0
            scored.append((is_clefable, len(mon.energy), self._max_hp(me, mon), idx))
        if not scored:
            return None
        scored.sort(reverse=True)
        return scored[0][3]
    ...
    def _main_mewtwo(self, me: Player) -> Pokemon | None:
        ...
        return max(
            mons,
            key=lambda m: (
                1 if self._belt_on(me, m) else 0,
                len(m.energy),
                0 if m is me.active else 1,
            ),
        )

    def _support_mewtwo(self, me: Player) -> Pokemon | None:
        main = self._main_mewtwo(me)
        for mon in self._mewtwo_mons(me):
            if mon is not main:
                return mon
    ...
    def _best_tank_idx(self, me: Player, require_survive: bool = False) -> int | None:
        ...
        Prefer a body that survives 140. Mewtwo is last among sponges — Photon is the
        closer. We can rotate Mega / Clefable ex; Charm Ogerpon wants to stay Active.
        """
        scored: list[tuple[int, int, int, int]] = []
        for idx, mon in enumerate(me.bench):
            if not self._is_tank_mon(me, mon):
    ...
    def _end_on_tank(self, me: Player, foe: Player, who: str) -> bool:
        ...
        if self._photon_ko(me, foe):
            return False
        if self._want_empty_clefairy_chump(me, foe):
            return True
        if self._want_three_two_combo(me, foe):
            return False
        if (
    ...
    def _want_fast_line(self, me: Player, foe: Player, who: str) -> bool:
        ...
        if self._want_storm_line(me, foe, who):
            return False
        # Vs D Clefairy-battery line: do not Transfer onto a Mewtwo that still has
        # to soak Demolish — the support copy Transfers after the 1-prize KO.
        if (
            self._want_empty_clefairy_chump(me, foe)
            or self._want_three_two_combo(me, foe)
            or self._support_transfer_turn(me, foe)
        ):
            return False
        return bool(
    ...
    def _wonder_storm_attack(self, me: Player, mon: Pokemon):
        card = me.card(mon.card_i)
        return next((a for a in card.attacks if "wonder storm" in a.name.lower()), None)

    def _can_pay_wonder_storm(self, me: Player, mon: Pokemon) -> bool:
        atk = self._wonder_storm_attack(me, mon)
        return bool(atk) and can_pay_energy(self._energy_pool(me, mon), atk.cost)
    ...
    def _evolve_party(self, me: Player, foe: Player, who: str) -> None:
        if self.rules.first_turn_no_evolve and self._is_players_first_turn(who):
            return
        if self._facing_aura(me):
            self._evolve_party_vs_aura(me, who)
            return
        engines = self._party_engines(me)
        ...
        if self._foe_can_demolish(foe) and not self._facing_demolish(me):
            mewtwo = self._mewtwo_mon(me)
    ...
    def _should_transfer_combo(self, me: Player, foe: Player) -> bool:
        who = "a" if me.name == "A" else "b"
        if self._want_empty_clefairy_chump(me, foe) or self._support_transfer_turn(me, foe):
            return False
        if self._photon_ko(me, foe):
            return False
    ...
    def _retreat_party(self, me: Player, foe: Player, who: str) -> None:
        ...
        if self._want_three_two_combo(me, foe):
            if self._photon_ko(me, foe):
                ...
                if killer is not None and killer is not me.active:
                    for idx, mon in enumerate(me.bench):
                        if mon is killer:
                            self._swap_to_bench(me, who, idx, allow_paid=True)
                            return
            ...
            if self._support_transfer_turn(me, foe):
                return
            if self._ogerpon_threat(foe) and not self._is_fodder_mon(me, me.active):
                fodder_idx = self._best_fodder_idx(me)
                if fodder_idx is not None:
                    self._swap_to_bench(me, who, fodder_idx, allow_paid=True)
                    return
            if self._is_fodder_mon(me, me.active):
                return
            if me.active and self._is_clefairy(me.card(me.active.card_i)):
                return
        if self._support_transfer_turn(me, foe):
        ...
        if self._photon_ko(me, foe):
            ...
            for idx, mon in enumerate(me.bench):
                if mon is killer:
                    ...
                    return
        if self._facing_aura(me):
            self._retreat_party_vs_aura(me, foe, who)
            return
        if self._facing_demolish(me):
            if me.active and self._is_mewtwo(me.card(me.active.card_i)) and not self._survives_demolish(me, me.active):
                for idx, mon in enumerate(me.bench):
                    if self._is_mewtwo(me.card(mon.card_i)) and self._survives_demolish(me, mon):
                        self._swap_to_bench(me, who, idx, allow_paid=True)
                        return
            if self._ogerpon_threat(foe):
                self._end_on_tank(me, foe, who)
    ...
    def _slash_energy_target(self, me: Player) -> Pokemon:
        ...
        cats = [
            m
            for m in me.in_play()
            if self._is_floragato(me.card(m.card_i)) or self._is_sprigatito(me.card(m.card_i))
        ]
        unpaid = [m for m in cats if not can_pay_energy(self._energy_pool(me, m), ["Grass", "Colorless"])]
        if unpaid:
    ...
    def _evolve_slash(self, me: Player, who: str) -> None:
        if self.rules.first_turn_no_evolve and self._is_players_first_turn(who):
            return
        evo_i = next((i for i in me.hand if self._is_floragato(me.card(i))), None)
        if evo_i is None:
            return
        evo = me.card(evo_i)
        candidates = [
    ...
    def _retreat_slash(self, me: Player, foe: Player, who: str) -> None:
        if not me.active or not me.bench:
            return
        if self._slash_ko(me, foe, me.active):
            return
        for idx, mon in enumerate(me.bench):
            if self._slash_ko(me, foe, mon):
                self._swap_to_bench(me, who, idx, allow_paid=True)
    ...
    def _coin_prevents_attack_damage(self, owner: Player, mon: Pokemon) -> bool:
        if self._abilities_suppressed(owner, mon):
            return False
        for abi in owner.card(mon.card_i).abilities:
            if any(e.get("kind") == "coin_prevent_attack_damage" for e in self._ability_effects(abi)):
                return self.rng.random() < 0.5
        return False

    def _recycle_energy_from_discard(self, me: Player, count: int = 2) -> None:
        fuels = [i for i in me.discard if me.card(i).is_energy]
        if not fuels:
            return

        def rank(idx: int) -> int:
            et = (me.card(idx).energy_type or me.card(idx).as_energy_type or "").lower()
            if et == "psychic":
                return 0
            if et == "darkness":
                return 1
        ...
        fuels.sort(key=rank)
        taken = 0
        for card_i in fuels[: max(1, count)]:
            me.discard.remove(card_i)
            me.hand.append(card_i)
            taken += 1
        if taken:
    ...
    def _evolve_from_deck(self, me: Player, foe: Player, who: str) -> None:
        ...
        self._do_evolve(me, target, evo_i)
        self._bump("expert_nurturer")
        self.rng.shuffle(me.deck)

    def _self_bench_damage(self, me: Player, amount: int) -> None:
    ...
    def _move_opp_active_energy_to_bench(self, foe: Player) -> None:
        ...
        dest = min(foe.bench, key=lambda m: len(m.energy))
        dest.energy.append(energy_i)
        self._bump("jamming_wing")
        self._log(f"Jamming Wing moves energy onto {foe.card(dest.card_i).name}")

    def _plusle_mon(self, me: Player) -> Pokemon | None:
        for mon in me.in_play():
            if me.card(mon.card_i).name.lower() == "plusle":
                return mon
        return None

    def _plus_damage_attack(self, card: Card):
        for atk in card.attacks:
            if any(e.get("kind") == "damage_counter_bonus" for e in atk.effects) or atk.name.lower() == "plus damage":
                return atk
        return None

    def _g_plusle_kos(self, me: Player, foe: Player, mon: Pokemon, extra_energy: int = 0) -> bool:
        if not foe.active:
            return False
        atk = self._plus_damage_attack(me.card(mon.card_i))
        if atk is None:
            return False
        pool = self._energy_pool(me, mon) + (["Colorless"] * max(0, extra_energy))
        if not can_pay_energy(pool, atk.cost):
            return False
        hp = self._max_hp(foe, foe.active) - foe.active.damage
        return self._raw_attack_damage(me, foe, mon, atk) >= hp > 0

    def _iron_boulder_mon(self, me: Player) -> Pokemon | None:
        for mon in me.in_play():
            if me.card(mon.card_i).name.lower() == "iron boulder":
                return mon
        return None

    def _adjusted_horn(self, card: Card):
        for atk in card.attacks:
            if any(e.get("kind") == "require_equal_hands" for e in atk.effects):
                return atk
        return None

    def _g_horn_can_pay(self, me: Player, mon: Pokemon) -> bool:
        horn = self._adjusted_horn(me.card(mon.card_i))
        if horn is None:
            return False
        pool = self._energy_pool(me, mon)
        if can_pay_energy(pool, horn.cost):
            return True
        if me.energy_attached:
            return False
        for i in me.hand:
            extra = energy_provided(me.card(i))
            if extra and can_pay_energy(pool + extra, horn.cost):
                return True
        return False

    def _g_storm_would_ko(self, me: Player, foe: Player) -> bool:
        if not foe.active:
            return False
        hp = self._max_hp(foe, foe.active) - foe.active.damage
        for mon in me.in_play():
            if not self._is_clefairy(me.card(mon.card_i)) or not self._can_pay_wonder_storm(me, mon):
                continue
            storm = next((a for a in me.card(mon.card_i).attacks if "wonder storm" in a.name.lower()), None)
            if storm and self._raw_attack_damage(me, foe, mon, storm) >= hp > 0:
                return True
        return False

    def _g_can_match_hands(self, me: Player, foe: Player) -> bool:
        need = len(foe.hand)
        have = len(me.hand)
        if have == need:
            return True
        names = [me.card(i).name.lower() for i in me.hand]
        if have > need:
            dump = names.count("potion") + names.count("energy switch")
            if "ultra ball" in names and len(me.hand) >= 3:
                dump += 2
            dump += sum(1 for i in me.hand if me.card(i).is_basic and me.card(i).name.lower() != "clefairy")
            if not me.energy_attached:
                dump += 1
            return have - dump <= need
        if any("iris" in n and "fighting spirit" in n for n in names) and need <= 6:
            return True
        nrg_disc = sum(1 for i in me.discard if me.card(i).is_energy)
        if "energy retrieval" in names and nrg_disc and have + 1 >= need:
            return True
        psychic_disc = sum(
            1
            for i in me.discard
            if (me.card(i).is_energy and (me.card(i).energy_type or "") == "Psychic")
            or (me.card(i).is_pokemon and me.card(i).types and me.card(i).types[0] == "Psychic")
        )
        if "tulip" in names and have - 1 + min(4, psychic_disc) >= need:
            return True
        if "drayton" in names and have + 1 >= need:
            return True
        return False

    def _g_wants_horn_swing(self, me: Player, foe: Player, who: str) -> bool:
        if self.strats[who].name != "g" or not foe.active:
            return False
        boulder = self._iron_boulder_mon(me)
        if boulder is None or not self._g_horn_can_pay(me, boulder):
            return False
        if self._g_storm_would_ko(me, foe):
            return False
        horn = self._adjusted_horn(me.card(boulder.card_i))
        if horn is None:
            return False
        dmg = self._raw_attack_damage(me, foe, boulder, horn, ignore_hand_gate=True)
        hp = self._max_hp(foe, foe.active) - foe.active.damage
        if dmg <= 0:
            return False
        if me.active is boulder:
            return True
        if dmg >= hp > 0:
            return True
    ...
    def _g_horn_attach(self, me: Player, foe: Player, who: str) -> bool:
        """Attach or skip for Adjusted Horn. True means this function handled the attach step."""
        boulder = self._iron_boulder_mon(me)
        if boulder is None:
            return False
        horn = self._adjusted_horn(me.card(boulder.card_i))
        if horn is None:
            return False
        payable = can_pay_energy(self._energy_pool(me, boulder), horn.cost)
        if not payable:
            if self._g_storm_would_ko(me, foe):
                return False
            energy_i = self._choose_energy_card(me, boulder, self.strats[who])
            if energy_i is None:
                return False
            me.hand.remove(energy_i)
            boulder.energy.append(energy_i)
            me.energy_attached = True
            src = me.card(energy_i)
            self._log(f"{me.name} attaches {src.name} as {src.as_energy_type} energy to {me.card(boulder.card_i).name}")
            if src.is_pokemon:
                self._bump("pokemon_as_energy")
            return True
        if not self._g_wants_horn_swing(me, foe, who):
            return False
        gap = len(me.hand) - len(foe.hand)
        if gap == 0:
            return True
        if gap != 1:
            return True
    ...
    def _g_horn_trainer_score(self, me: Player, foe: Player, who: str, card_i: int) -> float:
        pending = 0
        boulder = self._iron_boulder_mon(me)
        if boulder is not None:
            horn = self._adjusted_horn(me.card(boulder.card_i))
            payable = horn is not None and can_pay_energy(self._energy_pool(me, boulder), horn.cost)
            if not payable and not me.energy_attached:
                pending = 1
            elif payable and not me.energy_attached and len(me.hand) - 1 == len(foe.hand):
                pending = 1
        target = len(foe.hand)
        before = abs(len(me.hand) - pending - target)
        after_hand = self._g_horn_hand_after_trainer(me, foe, who, card_i)
        if after_hand is None:
            return -8
        after = abs(after_hand - pending - target)
    ...
    def _g_horn_hand_after_trainer(self, me: Player, foe: Player, who: str, card_i: int) -> int | None:
        card = me.card(card_i)
        name = card.name.lower()
        h = len(me.hand) - 1
        if name == "potion":
        ...
        if name == "energy search":
            return h + 1 if any(is_basic_energy(me.card(i), pokemon_as_energy=self.rules.pokemon_as_energy) for i in me.deck) else h
        if name in {"trekking shoes"}:
            return h + 1 if me.deck else h
        if name == "energy retrieval":
            n = min(2, sum(1 for i in me.discard if me.card(i).is_energy))
            return h + n
        if name == "tulip":
            n = min(
                4,
        ...
        if name == "ultra ball":
            ...
            return h - 2 + 1
        if name == "surfer":
            if me.active and me.card(me.active.card_i).name.lower() == "iron boulder":
                return None
            return 5 if h < 5 else h
        if "iris" in name and "fighting spirit" in name:
            after_disc = h - 1
            return 6 if after_disc < 6 else after_disc
        if name == "drayton":
            return h + 1
        if name in {"poké ball", "poke ball"}:
            return h + 1
        return h

    def _g_play_trainer_i(self, me: Player, foe: Player, who: str, found: int) -> bool:
        card = me.card(found)
        if card.is_supporter:
            if me.supporter_used or not self._can_play_supporter(who):
                return False
        ...
        if card.name.lower() not in {"ultra ball"}:
            me.discard.append(found)
        self._resolve_trainer(me, foe, card, who=who, card_i=found)
        self._log(f"{me.name} plays {card.name} (horn match)")
        return True

    def _g_horn_dump_one(self, me: Player, foe: Player, who: str) -> bool:
        prefer = ("potion", "energy switch", "ultra ball")
        names = {i: me.card(i).name.lower() for i in me.hand}
        for want in prefer:
            if want == "ultra ball" and len(me.hand) < 3:
                continue
            hit = next((i for i, n in names.items() if n == want), None)
            if hit is not None:
                return self._g_play_trainer_i(me, foe, who, hit)
        protect = {n.lower() for n in self.strats[who].protect}
        for i in list(me.hand):
            card = me.card(i)
            ...
            self._bump(f"saw_play:{card.name}")
            self._log(f"{me.name} benches {card.name} (horn dump)")
            return True
        return False

    def _g_horn_draw_toward(self, me: Player, foe: Player, who: str, target: int) -> bool:
        names = {i: me.card(i).name.lower() for i in me.hand}
        gap = target - len(me.hand)
        if gap <= 0:
            return False
        if target <= 6:
            iris = next((i for i, n in names.items() if "iris" in n and "fighting spirit" in n), None)
            if iris is not None and len(me.hand) >= 2:
                return self._g_play_trainer_i(me, foe, who, iris)
        if gap == 1:
            retr = next((i for i, n in names.items() if n == "energy retrieval"), None)
            if retr is not None and any(me.card(i).is_energy for i in me.discard):
                return self._g_play_trainer_i(me, foe, who, retr)
        tulip = next((i for i, n in names.items() if n == "tulip"), None)
        if tulip is not None:
            return self._g_play_trainer_i(me, foe, who, tulip)
        drayton = next((i for i, n in names.items() if n == "drayton"), None)
        if drayton is not None:
            return self._g_play_trainer_i(me, foe, who, drayton)
        return False

    def _g_match_horn_hands(self, me: Player, foe: Player, who: str) -> None:
        if not me.active or me.card(me.active.card_i).name.lower() != "iron boulder":
            return
        if not self._g_horn_can_pay(me, me.active):
            return
        target = len(foe.hand)
        for _ in range(8):
            gap = len(me.hand) - target
            if gap == 0:
                self._bump("horn_match")
                return
            if gap > 0:
                if not self._g_horn_dump_one(me, foe, who):
                    return
                continue
            if not self._g_horn_draw_toward(me, foe, who, target):
                return
        if len(me.hand) == target:
            self._bump("horn_match")

    def _g_best_hit(self, me: Player, foe: Player, mon: Pokemon) -> tuple[bool, int]:
        if not foe.active:
            return False, 0
        hp = self._max_hp(foe, foe.active) - foe.active.damage
        attached = self._energy_pool(me, mon)
        locked = mon.disabled_attack
        best = 0
        ko = False
        for atk in me.card(mon.card_i).attacks:
            if atk.name == locked or not can_pay_energy(attached, atk.cost):
                continue
            gate = any(e.get("kind") == "require_equal_hands" for e in atk.effects)
            dmg = self._raw_attack_damage(
                me,
                foe,
                mon,
                atk,
                ignore_hand_gate=bool(gate and self._g_can_match_hands(me, foe)),
            )
            if gate and not self._g_can_match_hands(me, foe):
                dmg = 0
            best = max(best, dmg)
            if dmg >= hp > 0:
                ko = True
    ...
    def _g_wants_flutter_shutoff(self, foe: Player) -> bool:
        if not foe.active:
            return False
        return bool(foe.card(foe.active.card_i).abilities)

    def _g_incoming_idx(self, me: Player, who: str) -> int | None:
        foe = self.players["b" if who == "a" else "a"]
        if not me.active or not me.bench:
            return None
        active_ko, _active_dmg = self._g_best_hit(me, foe, me.active)
        best_ko: int | None = None
        best_ko_dmg = -1
        for idx, mon in enumerate(me.bench):
            ko, dmg = self._g_best_hit(me, foe, mon)
            if ko and dmg > best_ko_dmg:
                best_ko = idx
                best_ko_dmg = dmg
        if best_ko is not None and not active_ko:
            return best_ko
        if active_ko:
            return None
        unused_party = (
        ...
        if self._g_wants_horn_swing(me, foe, who):
            for idx, mon in enumerate(me.bench):
                if me.card(mon.card_i).name.lower() != "iron boulder":
                    continue
                horn = self._adjusted_horn(me.card(mon.card_i))
        ...
        if unused_party:
            return None
        storm: list[tuple[int, int]] = []
        for idx, mon in enumerate(me.bench):
            if self._is_clefairy(me.card(mon.card_i)) and self._can_pay_wonder_storm(me, mon):
                storm.append((self._psychic_on(me, mon) + len(mon.energy), idx))
        if storm:
            storm.sort(reverse=True)
            if not self._can_pay_wonder_storm(me, me.active):
                return storm[0][1]
    ...
    def _retreat_g(self, me: Player, foe: Player, who: str) -> None:
        idx = self._g_incoming_idx(me, who)
        if idx is None:
            return
        incoming = me.bench[idx]
