...
- Do not put CLC Metronome back in this 2-of slot unless the plan is specifically to copy Dive on a 70 HP body. 2026-09-22 1-of trial (no Pad): T60 67.2 → 65.5. Combined with 1 Poké Pad as a Clefairy / Prankish / Metronome superposition (1 Prankish + 1 CLC + 1 Pad −1 Hop): the Pad **does** fetch CLC vs Dragapult (812 / 3,000 T60 games) and T60 still drops 67.7 → 63.9. Paying for CLC with Clefable ex or a Psychic Energy instead of Hop also loses (T60 65.0 / 65.0 vs pad-only 68.0). Ultra Ball and Mega are the only payments that hold vs pad-only, and Mega still loses T60. Pad-only (1 Prankish + 1 Pad, no copy) is 68.0 / wComp 69.3. The lock stays 2 Prankish. See [set-c60-pad-clc-cuts.md](set-c60-pad-clc-cuts.md), [set-c60-pad-metronome.md](set-c60-pad-metronome.md), and [set-c60-metronome.md](set-c60-metronome.md).
