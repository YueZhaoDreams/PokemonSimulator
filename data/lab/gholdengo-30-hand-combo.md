# Boltund V Bolt Storm — 60-card Unlimited (G30 lab)

Date: 2026-09-17
Status: engine plays Boltund V Rebel Clash 67 Bolt Storm (Lightning Basic V, Electrify / Bolt Storm 10+30x Lightning in play) with Voltaic Lightning Energy Pitch Black 84 (+20 each on a Lightning host, counts as Lightning attached), Lopunny FLF Big Jump, Shuckle HGSS Promo 15 Fermenting Liquid (every from-hand attach draws extra), Shaymin UL Celebration Wind recycling Speed Lightning Energy, Penny / Scoop Up Net to replay Shaymin, Porygon BTS line (Porygon2, Candy backup), and Draw Energy. No Galarian Meowth, no Metal Energy, no Iron Hands ex, no VMAX. Win-rate array vs household 60s is in `data/lab/gholdengo-30-array.json`.
...
**主攻再改：Boltund V RCL 67（`swsh2-67`），VMAX 不考虑（还要进化）。** 同名 4 张对换，60 数量不动。**Electrify** `[L]` 从牌库贴最多 2 张**基础** Lightning 到后排（特殊电搜不出）；**Bolt Storm** `[L][C]` 10+30×**全场 Lightning**（贴着的特殊电也算），Voltaic 再 +20/张。引擎三改：付完继续堆 Lightning（每张 +30）、Storm 秒不掉就 Electrify 铺场、基础电用完就 Storm 白嫖磨血（无副作用，不 pass）。退 2，绝不撤退。这版数组 **18.7 / 10.1 / 13.3 / 30.0 / 27.5 / 55.0 / 62.3%**：c60 1.3→18.7、d60 1→27.5、s60 2.5→55、g 36.5→62.3，Storm 开火 44–72%、Electrify 铺场 35–64%。剩下天花板是 Dragapult 320：11 电全贴+Voltaic 才 340，t60/hedrick 经常先把 200 HP 的 V 点死。
...
## The 60 (Unlimited constructed, Bolt Storm)

Printings are the text this lab uses. Boltund V in this 60 is **Rebel Clash 67** (Electrify / Bolt Storm), not Fusion Strike Electrobullet (FST 103 stays reachable by alias). Voltaic Lightning Energy is **Pitch Black 84**. Draw Energy is Cosmic Eclipse 209.
...
| 4 | Boltund V | RCL 67 | **Bolt Storm** `[L][C]` 10+30×Lightning in play. **Electrify** `[L]` attaches up to 2 Basic Lightning from deck to Bench (basics only — Speed/Voltaic stay). Lightning Basic V, 200 HP, retreat 2, **2 prizes** — do not open on it; never retreat it (retreat dumps the stack). Cap **1** in play (bench seat for Shaymin). Hunt still wants a spare in hand. 8 Lightning = **250** (Mewtwo 230); 11 + Voltaic = **340** (Dragapult 320). |
...
| 1 | Switch | any | Into paid Boltund V when Bolt Storm KOs. Do not retreat Boltund V (retreat 2 dumps the Lightning stack). |
...
| 4 | Draw Energy | CEC 209 | Colorless; attach from hand, draw 1. On Shuckle, Fermenting adds 1 more (net **+1**). Crazy Code can spam it. Pays the Colorless part of Bolt Storm if Boltund V is still unpaid. |
| 3 | Lightning Energy | any | Basic Lightning. Pays `[L]` parts; stays on Boltund V through Wind. Cannot Crazy Code. Only these 3 are Electrify targets. |
...
Goldfish (board already up, going second): BTS Porygon into Porygon2 into Z (or Candy), BTS into Lopunny, Nest Boltund V. Hold Shuckle until Crazy Code can attach Draw Energy (or Enriching + Wind). Attach Lightning to Boltund V (keep stacking past paid — every Lightning is +30). Crazy Code Speed L onto Boltund (draw 2 each, up to +8), Voltaic onto Boltund (+30 count +20 bonus each), Draw Energy onto Shuckle (draw 1+1 each), Enriching onto Shuckle if Shaymin is in hand (draw 4+1) else Lopunny (draw 4). Play Shaymin from hand: Celebration Wind moves Speed L / Draw / Enriching (not Voltaic/Lightning) onto Lopunny — skip Wind if Bolt Storm already KOs. Big Jump returns Buneary + Lopunny + Speed L + Enriching + Draw Energy. Replay Buneary, BTS evolve, Net or Penny Shaymin, attach again. Bolt Storm 10+30×Lightning (+20/Voltaic on the attacker); Electrify when Storm would not KO and basics sit in the deck; Storm chips (no downside) once Electrify is spent.
...
G30 (`celebration` strategy, Boltund V RCL Bolt Storm + 4 Voltaic Lightning Energy + Lopunny FLF Big Jump + 2 Shuckle HGSS Promo 15 Fermenting Liquid, attach-ready bench + 4 Shaymin UL Celebration Wind + Penny + 2 Porygon2 BTS line; **0 Galarian Meowth**, **0 Metal Energy**, **0 Iron Hands ex**) as player A vs the household 60s. Not a full NxN remake of `set-c60-unl-matrix`. Rules preset `s60` (60 / 6 prizes / 4-of). First player random. **3,000 games / cell, seed 20260911**. Elapsed **87.9s**.

Script: `data/lab/gholdengo-30-array.py`. Numbers: `data/lab/gholdengo-30-array.json`.

| A \\ B | c60 | t60 | hedrick | unl | d60 | s60 | g |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| g30 win | 18.7% | 10.1% | 13.3% | 30.0% | 27.5% | 55.0% | 62.3% |
| first | 19.5% | 11.2% | 13.1% | 30.5% | 30.0% | 54.5% | 62.4% |
| second | 17.9% | 9.1% | 13.5% | 29.6% | 25.1% | 55.6% | 62.1% |
| Speed L draw | 30.7% | 19.8% | 24.3% | 28.7% | 14.0% | 43.7% | 24.9% |
| Big Jump | 34.6% | 27.1% | 27.7% | 33.5% | 23.0% | 38.1% | 18.8% |
| Celebration Wind | 26.3% | 20.8% | 21.9% | 25.1% | 16.8% | 29.9% | 13.9% |
| Bolt Storm | 54.7% | 44.5% | 49.2% | 49.1% | 56.5% | 71.8% | 72.4% |
| Electrify | 55.0% | 49.6% | 46.5% | 50.2% | 64.3% | 57.0% | 35.5% |
| Electrify attach | 55.0% | 49.6% | 46.3% | 50.2% | 64.3% | 57.0% | 35.2% |
| Penny | 26.9% | 20.0% | 22.0% | 21.7% | 17.8% | 29.3% | 16.7% |
| Scoop Up Net | 29.7% | 22.8% | 24.9% | 25.0% | 21.6% | 32.6% | 17.9% |
| Fermenting Liquid | 30.7% | 21.8% | 24.2% | 27.6% | 16.4% | 39.0% | 21.3% |

Same seed, previous **Boltund V FST Electrobullet**: g30 win **1.3 / 7.5 / 10.4 / 22.0 / 1.0 / 2.5 / 36.5%**, Electrobullet **20.1 / 6.8 / 16.0 / 15.2 / 0.0 / 19.8 / 41.1%**, Smash Turn **5.4 / 25.9 / 28.9 / 12.4 / 0.0 / 0.2 / 36.2%**. Swapping FST→RCL (same 60 otherwise, plus stack-past-paid + Electrify-fallback + Storm-chip) lifts every cell: **c60** 1.3→18.7, **d60** 1.0→27.5, **s60** 2.5→55.0, **g** 36.5→62.3, **unl** 22.0→30.0, **hedrick** 10.4→13.3; **t60** 7.5→10.1 barely moves. Bolt Storm fires 44–72% and Electrify ramps in 35–64%; the chip fallback (Storm has no downside, so it 2HKOs instead of passing once basics run out) roughly doubled c60/d60. Remaining ceiling is Dragapult 320: even a perfect 11-Lightning + Voltaic stack is 340, and t60/hedrick KO the 200 HP V before the stack lands.
...
Goldfish from a ready board still grows the hand (Big Jump returns Buneary + Lopunny + Enriching + Draw Energy after Wind; Draw Energy on Shuckle is net +1; Speed L on Boltund V is +2) and Bolt Storm fires **if `[L][C]` is attached**, scaling 10+30×Lightning (+20/Voltaic on the attacker). Live vs household 60s Draw Energy / Candy / Fermenting still fire. Stacked Storm KOs Mewtwo 230 (8 Lightning) and Ogerpon 210; Dragapult 320 needs a near-perfect 11-Lightning stack. Boltund V is 200 HP / 2 prizes — do not open on it.

Household 60s also kill the board first. C60 Photon Kinesis KOs 90 HP Lopunny / 130 HP Porygon-Z for one prize each and 2-shots Boltund V. d60 Dive / Ogerpon is faster than stacking 8 Lightning. t60/hedrick Dragapult 320 out-races the full 340 stack most games.

---

## Win condition

**Current closer — Boltund V RCL 67.** Lightning Basic V, 200 HP, Fighting ×2, retreat 2, **2 prizes**. Catalog `swsh2-67`. Speed L host. Do not open on it. Never retreat it.

- **[L] Electrify** — Search your deck for up to 2 Lightning Energy cards and attach them to your Benched Pokémon in any way you like. Then, shuffle your deck. Basics only (Speed/Voltaic are Special, unsearchable). Fires when Bolt Storm would not KO and basics sit in the deck.
- **[L][C] Bolt Storm 10+30×** — This attack does 30 more damage for each Lightning Energy attached to all of your Pokémon. Special Lightning counts once attached. Voltaic adds 20 more each on the attacker. No downside, so it chips toward the 2HKO instead of passing once Electrify is spent. 8 Lightning = **250** (Mewtwo 230, Ogerpon 210); 11 + Voltaic = **340** (Dragapult 320).

**Previous closer — Boltund V FST 103.** Catalog `swsh8-103`, retreat 1. Smash Turn `[L]` 30 switches; Electrobullet `[L][L][C]` 120 + 30 bench snipe. 4 Voltaic capped at **200** — missed every tank. Kept as the `boltund v fst` alias for the snipe parser test.
