#!/usr/bin/env python3
"""Gholdengo 30-hand 60 vs household 60s. Directed array, not a full NxN remake.
...
def main() -> None:
    ...
    with ProcessPoolExecutor(max_workers=min(7, len(keys))) as pool:
        ...
        for fut in as_completed(futs):
            ...
            print(
                ...
                f"30-hand {detail['hand_thirty']:.1%} celebration {detail['celebration']:.1%})",
                flush=True,
            )
    ...
    print("| 30-hand | " + " | ".join(f"{cells[k]['hand_thirty']:.1%}" for k in keys) + " |")
    print("| Celebration | " + " | ".join(f"{cells[k]['celebration']:.1%}" for k in keys) + " |")
